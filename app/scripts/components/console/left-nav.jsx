...
const LeftNav = React.createClass({
    getInitialState(){
        return {
            ...
            data: [],
            stations: [],
            stationActive: {name: '紧水滩水电站', id: 's001'}
        }
    },
    componentWillMount(){
        /* Get config */
    ...
    handleSelectStation(id, name){
        /* Change the station */
        // TODO
        if(this.state.stationActive.id != id){
            this.setState({
                stationActive: {name: name, id: id}
            });
            this.props.handleSelectStation(id);
        }
    },
    handleSkimArea(){
        this.setState({
                        ...
                        <div className="stations-wrapper">
                            <ul className="stations">
                                {this.state.stations.map((item)=>{
                                  return(
                                      <li className="item" onClick={self.handleSelectStation.bind(this, item.id, item.name)}>{item.name}</li>
                                  );
                                })}
                            </ul>
                        </div> : null
